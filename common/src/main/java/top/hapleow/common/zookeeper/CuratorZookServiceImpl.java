 ...
 * 此处均为同步阻塞方法
 */
public class CuratorZookServiceImpl implements IZookService {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 创建持久节点
        client.create().withMode(CreateMode.PERSISTENT).forPath("/persistent", "created a persistent node.".getBytes());

        // 创建临时节点(断开连接后该节点会被删除)
        client.create().withMode(CreateMode.EPHEMERAL).forPath("/ephemeral", "created an ephemeral node.".getBytes());

        // 创建持久顺序节点（zookeeper自动分配递增后缀）
        for (int i = 0; i < 5; i++) {
            client.create().withMode(CreateMode.PERSISTENT_SEQUENTIAL).forPath("/persistent_sequential");
        }

        // 创建临时顺序节点（zookeeper自动分配递增后缀）
        for (int i = 0; i < 5; i++) {
            client.create().withMode(CreateMode.EPHEMERAL_SEQUENTIAL).forPath("/persistent_sequential");
        }
        ...
        byte[] persistentData = client.getData().forPath("/persistent");
        ...
        byte[] bytes = client.getData().forPath("/persistent");
